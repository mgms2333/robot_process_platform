...
int main()
{
    ...
    const std::string system_config_file_path = "./config/system_config.json";
    ...
    robot_process_platform::platform::SystemService system_service;

    const robot_process_platform::core::Status initialize_system_status =
        system_service.Initialize(system_config_file_path, false);
    if (!initialize_system_status.Ok())
    {
        robot_process_platform::platform::Logger::GetInstance().LogE("Runner", "Failed to initialize system service.");
        std::cout << "failed_to_initialize_system_service" << '\n';
        std::cout << "error_code=" << robot_process_platform::core::ToString(initialize_system_status.code) << '\n';
        if (!initialize_system_status.message.empty())
        {
            std::cout << "error_message=" << initialize_system_status.message << '\n';
        }
        robot_process_platform::platform::Logger::GetInstance().Shutdown();
        return 1;
    }
    robot_process_platform::platform::Logger::GetInstance().LogI("Runner", "System service initialized.");

    const robot_process_platform::core::Status connect_selected_robot_status =
        system_service.ConnectSelectedRobot();
    if (!connect_selected_robot_status.Ok())
    {
        robot_process_platform::platform::Logger::GetInstance().LogE("Runner", "Failed to connect selected robot.");
        std::cout << "failed_to_connect_selected_robot" << '\n';
        std::cout << "error_code=" << robot_process_platform::core::ToString(connect_selected_robot_status.code) << '\n';
        if (!connect_selected_robot_status.message.empty())
        {
            std::cout << "error_message=" << connect_selected_robot_status.message << '\n';
        }
        system_service.Shutdown();
        robot_process_platform::platform::Logger::GetInstance().Shutdown();
        return 1;
    }
    robot_process_platform::platform::Logger::GetInstance().LogI("Runner", "Selected robot connected.");

    const robot_process_platform::core::Status template_load_status =
        system_service.LoadTemplateLibrary(template_library_file_path);
    ...
    const robot_process_platform::core::Result<robot_process_platform::core::Task> task_creation_result =
        system_service.CreateTask(template_name,
    ...
    const robot_process_platform::core::Status start_task_status =
        system_service.StartTask(created_task.task_id, generated_task_directory_path, 0, 0);
    ...
    robot_process_platform::platform::Logger::GetInstance().LogI("Runner", "System StartTask completed.");
    ...
    robot_process_platform::runtime::ExecutionContext runtime_snapshot =
        system_service.GetRuntimeSnapshot();
    while (!IsTerminalState(runtime_snapshot.runtime_state))
    {
        std::this_thread::sleep_for(kRuntimePollInterval);
        runtime_snapshot = system_service.GetRuntimeSnapshot();
    }

    const robot_process_platform::platform::SystemState system_state =
        system_service.GetSystemState();
    system_service.Shutdown();
    ...
    std::cout << "active_robot_name=" << system_state.active_robot_name << '\n';
